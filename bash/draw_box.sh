#!/bin/bash
# draw_box.sh: Drawing a box using ASCII characters.
...
if [ $# -lt "$MINARGS" ]; then # if args are than 4, exit.
    exit $E_BADARGS
fi
...
P_ROWS=`expr $T_ROWS - 1` # Put prompt at bottom of the terminal.

echo -e "\E[${P_ROWS};1H"
}

# Let's try drawing a box
...
W=45 # Width
col=1 # Color (red)
draw_box $R $C $H $W $col # Draw a box
